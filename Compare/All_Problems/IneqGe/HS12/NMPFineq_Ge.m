...
% Begining
fid=fopen('ineq_Ge.txt','at');
[fxold,gfxold]=cutest_obj(xold);nf=1;ngf=1; % f(x), gradf(x)
[cxold,J]=cutest_cons(xold);gcxold=J';nc=1;ngc=1;% gcxold:n by mie
...
while ((iter<=nmax)&&(nf<=fvmax))
    ...
    if Rkcount==0 % Linearized constraints are compatible
        ...
        [dquad,fquad]=quadprog(Bk,subg,A2,subb2,[],[],v1,[],[],options);
        dopt=dquad(1:n);
    end        
    ...
    if FL<=MF && norm(cxold)+norm(J)<= 1e+10
        ...
        if alpha<=1e-8
            ...
            flag_Alg=2;break;%alpha is too small, go to the end and output the results.
        end
    end  % end for FL<=MF  
    % Updatation
