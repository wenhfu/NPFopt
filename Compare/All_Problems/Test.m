...
% function [x,fval,exitflag,output,lambda] = NPFopt_main(funf,func,con_type,x0,MF,opts)
clear;clc;
...
while ((iter <= itermax) && (nf <= nfmax))
    ...
    % if m_ineq == 0; A = zeros(m_ineq, n + 2*m_eq + m_ineq); b = zeros(m_ineq,1); end
    % if m_eq == 0; Aeq = zeros(m_eq, n + 2*m_eq + m_ineq); beq = zeros(m_eq,1); end
    ...
    if FL == 0
        ...
        Mu = pinv(gcxnew_eq)*gfxnew; Lam = pinv(gcxnew_ineq)*gfxnew;
...
if alg_display == 1
    ...
    fprintf('n =%4d, m =%4d, nf =%4d, ng =%4d, iter =%3d, f =%12.4e, v =%12.4e ',n,m,nf,ngf,iter,fxnew,vxnew);
end
